...
class ZeroProDetection:
    ...
    def modelConstruct(self, wordEmbeddingDim, dimLSTM, dimDense,  wordEmbeddingMatrix = [], droupoutRate = 0.05):
        self.logger.info('Start to construct the model...')
        # Set a empty model
        mainInput = Input(shape=(self.MAX_SEQUENCE_LENGTH,))
        if len(wordEmbeddingMatrix) > 0:
            embeddingOutput = Embedding(len(self.word_index) + 1, wordEmbeddingDim, weights = [wordEmbeddingMatrix], trainable = False, mask_zero = False)(mainInput)
        else:
            embeddingOutput = Embedding(len(self.word_index) + 1, wordEmbeddingDim, mask_zero = False)(mainInput)
        ...
        dpOutput = Dropout(droupoutRate) (mergeOutput)
        
        flattenOutput = Flatten() (dpOutput)
        
        denseOutput = Dense ( dimDense, activation = 'tanh') ( flattenOutput )
        denseSecOutput = Dense (dimDense, activation = 'tanh')  (denseOutput)
        
        finalOutput = Dense ( self.MAX_SEQUENCE_LENGTH, activation = 'sigmoid') ( denseSecOutput )
    ...
    def modelTraining(self, model, epoches, batchSize, valSplit):
        ...
        earlystop_cb = EarlyStopping(monitor='val_loss', patience= int(epoches * 0.1), verbose=0, mode='auto')    
        model.fit(self.trainData, self.trainLabels, nb_epoch = epoches, batch_size = batchSize, validation_split = valSplit, callbacks = [earlystop_cb]) 
        #save the model
        try:
            os.mkdir('model')
        except OSError:
...
if __name__ == '__main__':
    ...
    fpResult = open(resultFile, 'w')
    
    for realText, realLabel, predictLabel, in zip(processor.testText, processor.testLabels, predictLabel):
        fpResult.write(realText + '\n')                
